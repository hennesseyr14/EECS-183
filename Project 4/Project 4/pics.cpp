 ...
 * rjhenn
 *
 * Main program which creates menu for user and allows them
 * to load text files and write images.
 */
...
void writeFile(const Graphics& drawer)
{
    // get file name from user and concatenate with .bmp
    string fileName;
    cin >> fileName;
    fileName = fileName + ".bmp";

    // write image to file and display message
...
void loadFile(Graphics& drawer)
{
    // create new stream for file data
    ifstream file;
    ...
    // character to tell program which shape to read
    char shapeChar;

    // read in and draw each shape in image
    while (file >> shapeChar)
    {
        if (shapeChar == 'L')
        {
            Line line;
        ...
        // print error message if unable to read
        else
        {
            drawer.clear();
            string restOfLine;
            getline(file, restOfLine);
            cout << "Error in input file: " << shapeChar
                 << restOfLine << endl;
        }
    }
    file.close();
...
string tolower(string str)
{
    // iterate through array and use the tolower function
    // to make a character lowercase if necessary
