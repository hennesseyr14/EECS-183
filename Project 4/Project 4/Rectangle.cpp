 ...
 * rjhenn
 *
 * Implementations of constructors and member functions for Rectangle class.
 */
...
{
    // try to read in for one of the possible formats
    ins >> start >> end
        >> colorTopLeft
        >> colorTopRight
        >> colorBottomRight
    ...
    // if read-in fails clear fail bit and continue reading in the other format
    if (ins.fail())
    {
        ins.clear();
